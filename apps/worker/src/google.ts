...
export async function fetchYoutubeVideoMetrics(supabase: SupabaseClient, tenantId: string, videoId: string) {
  ...
  const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=statistics,contentDetails,snippet,status&id=${encodeURIComponent(videoId)}`, { headers: { Authorization: `Bearer ${token}` } });
