...
export async function uploadFileToDrive(supabase: SupabaseClient, tenantId: string, fileName: string, mimeType: string, data: Buffer, folderId?: string) {
  const token = await getAccessToken(supabase, tenantId, 'drive');
  const metadata = { name: fileName, mimeType, ...(folderId ? { parents: [folderId] } : {}) };
  const result = await uploadMultipart('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink', token, metadata, mimeType, data);
...
export async function uploadVideoToYouTube(supabase: SupabaseClient, tenantId: string, title: string, description: string, data: Buffer) {
  ...
  const result = await uploadMultipart('https://www.googleapis.com/upload/youtube/v3/videos?uploadType=multipart&part=snippet,status', token, metadata, 'video/mp4', data);
